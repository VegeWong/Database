...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if (pid.pgcateg() == BTreePageId.LEAF) {
            curPage = (BTreePage) getPage(tid, dirtypages, pid, perm);
            return (BTreeLeafPage) curPage;
        }
        else if (pid.pgcateg() == BTreePageId.INTERNAL) {
            curPage = (BTreePage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
            BTreeEntry curEntry = null;
            Iterator<BTreeEntry> itr = ((BTreeInternalPage) curPage).iterator();
            while (itr.hasNext()) {
                curEntry = itr.next();
            ...
            if (curEntry == null)
                throw new DbException("Can not find corresponding child in a empty internal page");
            return findLeafPage(tid, dirtypages, curEntry.getRightChild(), perm, f);
        }
        else
            throw new DbException("Can only visit internal and leaf pages in findLeafPage");
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        dirtypages.putIfAbsent(page.getId(), page);
        Iterator<Tuple> ritr = page.reverseIterator();
        Tuple t = null;
        for (int i = page.getNumTuples()/ 2; i > 0; --i) {
            if (ritr.hasNext())
                t = ritr.next();
            else
                throw new DbException("Split leaf page with empty slots");
            page.deleteTuple(t);
            newPage.insertTuple(t);
        }

        BTreeEntry newEntry = new BTreeEntry(t.getField(keyField), page.getId(), newPage.getId());
        ...
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, parentId, t.getField(keyField));
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        dirtypages.putIfAbsent(page.getId(), page);
        Iterator<BTreeEntry> ritr = page.reverseIterator();
        BTreeEntry curEntry = null;
        BTreeEntry newPgLeftEntry = null;
        for (int i = (page.getNumEntries() + 1) / 2; i > 0; --i) {
            if (ritr.hasNext())
                curEntry = ritr.next();
            else
                throw new DbException("Split internal page with empty slots");
            page.deleteKeyAndRightChild(curEntry);
            if (newPgLeftEntry != null)
                newPage.insertEntry(newPgLeftEntry);
            if (i != 1)
                newPgLeftEntry = curEntry;
        }
        newPgLeftEntry.setLeftChild(curEntry.getRightChild());
        newPage.updateEntry(newPgLeftEntry);
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int emptySlotsPg = page.getNumEmptySlots();
        int emptySlotsSi = sibling.getNumEmptySlots();
        if (entry.getRecordId().getPageId() != parent.getId())
            throw new DbException("Provided entry is not in the parent page");
        if (isRightSibling && !(entry.getLeftChild().equals(page.getId()) && entry.getRightChild().equals(sibling.getId())))
            throw new DbException("Entry child pointer conflicts with the value provided");
        else if (!isRightSibling && !(entry.getLeftChild().equals(sibling.getId()) && entry.getRightChild().equals(page.getId())))
            throw new DbException("Entry child pointer conflicts with the value provided");

        Tuple curTuple = null;
        if (isRightSibling) {
            Iterator<Tuple> itr = sibling.iterator();
            for (int i = 0; i < (emptySlotsPg - emptySlotsSi) / 2; ++i) {
                if (itr.hasNext())
                    curTuple = itr.next();
            ...
            Iterator<Tuple> itr = sibling.reverseIterator();
            for (int i = 0; i < (emptySlotsPg - emptySlotsSi) / 2; ++i) {
                if (itr.hasNext())
                    curTuple = itr.next();
                sibling.deleteTuple(curTuple);
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // some code goes here
        if (!parentEntry.getRecordId().getPageId().equals(parent.getId()))
            throw  new DbException("Given entry is not in the parent page");
        if (!parentEntry.getLeftChild().equals(leftSibling.getId()) ||
                !parentEntry.getRightChild().equals(page.getId()))
            throw new DbException("Entry child pointers conflict with description");

        int numLft = leftSibling.getNumEntries();
        int numRht = page.getNumEntries();

        Iterator<BTreeEntry> litr = leftSibling.reverseIterator();
        BTreeEntry le = null;
        BTreeEntry re = null;
        for (int i = 0; i < (numLft - numRht) / 2; ++i) {
            le = litr.next();
            leftSibling.deleteKeyAndRightChild(le);

            // pull
            re = new BTreeEntry(parentEntry.getKey(), le.getRightChild(), page.getChildId(0));
            page.insertEntry(re);

            // push
            parentEntry.setKey(le.getKey());
        }
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        if (!parentEntry.getLeftChild().equals(page.getId()) ||
                !parentEntry.getRightChild().equals(rightSibling.getId()))
            throw new DbException("Entry child pointers conflict with description");

        int numLft = page.getNumEntries();
        int numRht = rightSibling.getNumEntries();

        Iterator<BTreeEntry> ritr = rightSibling.iterator();
        BTreeEntry le = null;
        BTreeEntry re = null;
        for (int i = 0; i < (numRht - numLft) / 2; ++i) {
            re = ritr.next();
            rightSibling.deleteKeyAndLeftChild(re);

            // pull
            if (le == null) {
                Iterator<BTreeEntry> litr = page.reverseIterator();
                le = new BTreeEntry(parentEntry.getKey(), litr.next().getRightChild(), re.getLeftChild());
            }
            else
                le = new BTreeEntry(parentEntry.getKey(), le.getRightChild(), re.getLeftChild());
            page.insertEntry(le);

            // push
            parentEntry.setKey(re.getKey());
        }
        parent.updateEntry(parentEntry);
        updateParentPointers(tid, dirtypages, page);
    }
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        Tuple t;
        Iterator<Tuple> ritr = rightPage.iterator();
        while (ritr.hasNext()) {
            t = ritr.next();
        ...
        if (rrid != null)
            ((BTreeLeafPage) getPage(tid, dirtypages, rrid, Permissions.READ_WRITE)).setLeftSiblingId(leftPage.getId());
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        Iterator<BTreeEntry> ritr = rightPage.iterator();
        BTreeEntry le = null;
        BTreeEntry re = null;
        while (ritr.hasNext()) {
            re = ritr.next();
            rightPage.deleteKeyAndLeftChild(re);

            if (le == null) {
                Iterator<BTreeEntry> litr = leftPage.reverseIterator();
                le = new BTreeEntry(parentEntry.getKey(), litr.next().getRightChild(), re.getLeftChild());
            }
            else
                le = new BTreeEntry(parentEntry.getKey(), le.getRightChild(), re.getLeftChild());
            leftPage.insertEntry(le);

            parentEntry.setKey(re.getKey());
        }
        le = new BTreeEntry(parentEntry.getKey(), le.getRightChild(), re.getRightChild());
        leftPage.insertEntry(le);
        deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
